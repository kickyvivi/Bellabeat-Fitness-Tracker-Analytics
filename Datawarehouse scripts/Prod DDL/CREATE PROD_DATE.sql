...
SELECT 
  ...
  TO_NUMBER(
    TRIM(
      leading '0' 
      FROM 
        TO_CHAR(CurrDate, 'DDD')
    )
  ) AS Day_Num_of_Year, 
  UPPER(
    TO_CHAR(CurrDate, 'Mon') || '-' || TO_CHAR(CurrDate, 'YYYY')
  ) AS Month_ID, 
  ...
  MAX(
    ...
    PARTITION BY TO_CHAR(CurrDate, 'Mon')
  ) AS Month_Time_Span, 
  ...
  ) AS Month_Num_of_Year, 
  TO_CHAR(CurrDate, 'YYYY') AS Year_ID, 
  --31 AS Year_Time_Span,
  COUNT(*) OVER (
    PARTITION BY TO_CHAR(CurrDate, 'YYYY')
  ) AS Year_Time_Span, 
...
FROM 
  (
    ...
    FROM 
      dual -- Change for the number of days to be added to the table.
